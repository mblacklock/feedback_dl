...
class FunctionalTestBase(StaticLiveServerTestCase):
    ...
    def cohort_select_box_action(self, select_box, option=False):
        """Utility helper to select an option by visible text and wait until selected.

        Args:
            select_box: element id of the select box
            option: visible text of the option to select
        """
        import time
        select_box_el = self.wait.until(EC.presence_of_element_located((By.ID, select_box)))
        sel = Select(select_box_el)
        if option:
            sel.select_by_visible_text(option)
            # Dispatch a change and blur to ensure page listeners run exactly as a user interaction
            try:
                self.browser.execute_script("arguments[0].dispatchEvent(new Event('change')); arguments[0].blur();", select_box_el)
            except Exception:
                # best-effort; continue
                pass
            # Also explicitly call the page-level saveNow() function to ensure an immediate POST
            try:
                self.browser.execute_script('if (typeof saveNow === "function") { saveNow(); }')
            except Exception:
                pass
            # Wait for the autosave UI indicator to show saved so persistence has completed
            try:
                self.wait.until(EC.text_to_be_present_in_element((By.ID, 'save-status'), 'Saved'))
            except Exception:
                # If UI indicator didn't update in time, fall back to a short sleep and verify selection
                time.sleep(0.5)
            # ensure the control reflects the chosen option
            self.wait.until(lambda d: sel.first_selected_option.text == option)
...
class FunctionalTestBase(StaticLiveServerTestCase):
    ...
    def select_box_action(self, select_box, option=False):
        """Utility helper to select an option by visible text and wait until selected.
        ...
        if option:
            ...
            try:
                ...
                # fall back to a short sleep if UI didn't update in time
                time.sleep(0.5)
            # ensure the control reflects the chosen option
            self.wait.until(lambda d: sel.first_selected_option.text == option)
